 ...
 * REMEDIATION: Removed N+1 image lookups; consume withImages from getCustomerStats()
...
export function useCustomerData(options = {}) {
  ...
  const [stats, setStats] = useState({
    total: 0, active30d: 0, tagged: 0, withImages: 0
  });
  ...
  /**
   * Fetch all customer data with statistics (REMEDIATION: single consolidated fetch)
   */
  const fetchCustomers = async () => {
    try {
      // Fetch customers and stats in parallel (REMEDIATION: stats includes withImages)
      const [customersData, statsData] = await Promise.all([
        getAllCustomers(options),
        getCustomerStats()
      ]);

      // REMEDIATION: No longer need individual image lookups per customer
      // withImages count comes from indexed stats query

      if (isMountedRef.current) {
        setCustomers(customersData);
        setStats(statsData); // REMEDIATION: stats now includes withImages from indexed query
        setError(null);
      }
