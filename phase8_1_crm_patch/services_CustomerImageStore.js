...
import { db, setProfileImage } from './database.js';
...
/**
 * PHASE 8.1: Set image as profile image for customer
 * @param {string} customerId - Customer ID
 * @param {number} imageId - Image ID to set as profile
 * @returns {Promise<boolean>} Success status
 */
export async function setAsProfileImage(customerId, imageId) {
  try {
    // Get the image to set as profile
    const image = await getCustomerImage(imageId);
    if (!image) {
      throw new Error(`Image ${imageId} not found`);
    }

    // Set profile image URL
    const success = await setProfileImage(customerId, image.url);

    if (success) {
      console.log(`[CustomerImageStore] Set image ${imageId} as profile for customer: ${customerId}`);
    }

    return success;

  } catch (error) {
    console.error('[CustomerImageStore] Set as profile image failed:', error);
    return false;
  }
}
 ...
 * Auto-sets first image as profile image
 * @param {string} customerId
...
export async function storeCustomerImage(customerId, imageFile, filename) {
  try {
    ...
    const imageRecord = await addCustomerImage(customerId, {
      url,
      filename,
      thumbnailUrl
    });

    // Auto-set as profile image if customer doesn't have one
    const customer = await db.customers.get(customerId);
    if (customer && !customer.profileImageUrl) {
      await setProfileImage(customerId, url);
      console.log(`[CustomerImageStore] Auto-set first image as profile for customer: ${customerId}`);
    }
